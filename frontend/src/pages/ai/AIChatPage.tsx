...
export const AIChatPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-5xl mx-auto">
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-4 pr-2">
        {messages.length === 0 ? (
          <div className="text-center py-12 space-y-4">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Startup Operating System AI</h3>
            ...
            <div className="flex flex-wrap justify-center gap-2 max-w-lg mx-auto pt-2">
              <button
                ...
                className="text-xs px-3 py-1.5 rounded-lg bg-white dark:bg-slate-800/80 hover:bg-slate-50 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700/60 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white shadow-sm dark:shadow-none transition-colors"
              >
              ...
              <div
                className={`max-w-2xl rounded-2xl p-4 text-xs leading-relaxed ${
                  m.sender === 'user'
                    ...
                    : 'bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 text-slate-800 dark:text-slate-200 shadow-sm dark:shadow-none'
                }`}
              >
                {/* Agent Plan Decomposition if present */}
                {m.plan && (
                  <div className="mb-3 p-2.5 rounded-lg bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700/40 text-[11px] space-y-1">
                    ...
                    <p className="text-slate-700 dark:text-slate-300 whitespace-pre-wrap font-mono">{m.plan}</p>
                  </div>
                )}
                  ...
                  <div className="mt-4 space-y-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                    {m.pending_approvals.map((appr) => (
                      <div
                        ...
                        className="bg-amber-500/5 dark:bg-slate-900 border border-amber-500/40 rounded-xl p-3 space-y-2 shadow-lg"
                      >
                        ...
                        <div className="text-[11px] text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-950/60 p-2 rounded border border-slate-200 dark:border-slate-800 font-mono">
                          <div>
                ...
                {m.tokens_used !== undefined && (
                  <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700/40 flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400">
        ...
        {isLoading && (
          <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 p-2">
            <Sparkles className="w-3.5 h-3.5 animate-spin text-brand-500 dark:text-brand-400" />
      ...
      {/* Input Form */}
      <form onSubmit={(e) => handleSend(e)} className="pt-3 border-t border-slate-200 dark:border-slate-800 flex gap-2">
        <input
          type="text"
          placeholder="Ask the AI Manager (e.g. 'Report project health' or 'Create task...') "
          value={input}
          onChange={(e) => setInput(e.target.value)}
          className="flex-1 px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
          disabled={isLoading}
        />
