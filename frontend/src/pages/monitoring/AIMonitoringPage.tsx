  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
            ...
            {[
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-md font-medium transition-colors ${
                  days === t.val ? 'bg-brand-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
                {t.label}
              </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-1 shadow-sm dark:shadow-none">
          <span className="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
            <Cpu className="w-3.5 h-3.5 text-brand-500 dark:text-brand-400" /> Total Agent Runs
          </span>
        ...
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-1 shadow-sm dark:shadow-none">
          <span className="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
            <Zap className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400" /> Token Consumption
          </span>
          <div className="text-2xl font-bold text-slate-900 dark:text-white">
        ...
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-1 shadow-sm dark:shadow-none">
          <span className="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
            <DollarSign className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400" /> Estimated LLM Cost
          </span>
        ...
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-1 shadow-sm dark:shadow-none">
          <span className="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
            <Clock className="w-3.5 h-3.5 text-purple-500 dark:text-purple-400" /> Average Latency
          </span>
          <div className="text-2xl font-bold text-slate-900 dark:text-white">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {!telemetry?.tool_usage?.length ? (
                ...
                telemetry.tool_usage.map((t) => (
                  <tr key={t.tool_name} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="py-2.5 px-4 font-mono font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                      <Wrench className="w-3.5 h-3.5 text-brand-500 dark:text-brand-400" /> {t.tool_name}
                    </td>
                    <td className="py-2.5 px-4 font-semibold text-slate-800 dark:text-slate-200">{t.call_count}</td>
                    <td className="py-2.5 px-4 text-red-500 dark:text-red-400">{t.error_count}</td>
                    <td className="py-2.5 px-4">
                      <span className={t.error_rate_percent > 10 ? 'text-red-500 dark:text-red-400 font-bold' : 'text-slate-500 dark:text-slate-400'}>
              ...
              {!telemetry?.recent_executions?.length ? (
                ...
                telemetry.recent_executions.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="py-2.5 px-4 font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                      <Cpu className="w-3.5 h-3.5 text-purple-500 dark:text-purple-400" /> {r.agent_name}
                    </td>
                    <td className="py-2.5 px-4 text-slate-700 dark:text-slate-300 max-w-xs truncate" title={r.query_snippet}>
                      {r.query_snippet}
                    </td>
                    <td className="py-2.5 px-4">
                      <Badge
                    ...
                    <td className="py-2.5 px-4 text-slate-500 dark:text-slate-400 font-mono">{r.execution_time_ms}ms</td>
                    <td className="py-2.5 px-4 text-slate-500 dark:text-slate-400 font-mono">{r.tokens_used}</td>
