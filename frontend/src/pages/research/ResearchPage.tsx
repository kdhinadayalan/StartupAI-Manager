    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-900/60 border border-emerald-500/30 rounded-xl p-4 space-y-3">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {!swot?.STRENGTH.length ? (
                <div className="text-slate-600 text-xs py-4 text-center">No strengths logged</div>
              ) : (
                swot.STRENGTH.map((s) => (
                  <div key={s.id} className="bg-slate-800/60 p-2.5 rounded-lg text-xs space-y-1">
                    <span className="font-semibold text-white">{s.title}</span>
                    <p className="text-[11px] text-slate-400">{s.key_findings}</p>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="bg-slate-900/60 border border-amber-500/30 rounded-xl p-4 space-y-3">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {!swot?.WEAKNESS.length ? (
                <div className="text-slate-600 text-xs py-4 text-center">No weaknesses logged</div>
              ) : (
                swot.WEAKNESS.map((w) => (
                  <div key={w.id} className="bg-slate-800/60 p-2.5 rounded-lg text-xs space-y-1">
                    <span className="font-semibold text-white">{w.title}</span>
                    <p className="text-[11px] text-slate-400">{w.key_findings}</p>
                  </div>
                ))
              )}
            </div>
          ...
          {/* Opportunities */}
          <div className="bg-slate-900/60 border border-sky-500/30 rounded-xl p-4 space-y-3">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {!swot?.OPPORTUNITY.length ? (
                <div className="text-slate-600 text-xs py-4 text-center">No opportunities logged</div>
              ) : (
                swot.OPPORTUNITY.map((o) => (
                  <div key={o.id} className="bg-slate-800/60 p-2.5 rounded-lg text-xs space-y-1">
                    <span className="font-semibold text-white">{o.title}</span>
                    <p className="text-[11px] text-slate-400">{o.key_findings}</p>
                  </div>
                ))
              )}
            </div>
          ...
          {/* Threats */}
          <div className="bg-slate-900/60 border border-red-500/30 rounded-xl p-4 space-y-3">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {!swot?.THREAT.length ? (
                <div className="text-slate-600 text-xs py-4 text-center">No threats logged</div>
              ) : (
                swot.THREAT.map((t) => (
                  <div key={t.id} className="bg-slate-800/60 p-2.5 rounded-lg text-xs space-y-1">
                    <span className="font-semibold text-white">{t.title}</span>
                    <p className="text-[11px] text-slate-400">{t.key_findings}</p>
                  </div>
                ))
              )}
            </div>
      ...
      {competitors.length > 0 && (
        <Card title="Competitor Roster" subtitle={`${competitors.length} tracked market competitors`}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {competitors.map((c) => (
              <div key={c.competitor} className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-4 space-y-2">
                ...
                <div className="space-y-1.5 pt-2 border-t border-slate-700/40 text-xs text-slate-300">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg p-6 shadow-2xl relative">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800 mb-4">
              <h3 className="text-base font-bold text-white">Add Market Intelligence</h3>
