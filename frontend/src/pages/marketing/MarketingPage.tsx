      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-1 shadow-sm dark:shadow-none">
          <span className="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
            <DollarSign className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400" /> Total Ad Spend
          </span>
          <div className="text-2xl font-bold text-slate-900 dark:text-white">
            {formatCurrency(performance?.total_spend, currencyCode)}
          </div>
          <span className="text-[11px] text-slate-500 dark:text-slate-400">
            Budget: {formatCurrency(performance?.total_budget, currencyCode)}
          </span>
        </div>

        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-1 shadow-sm dark:shadow-none">
          <span className="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
            <MousePointer className="w-3.5 h-3.5 text-brand-500 dark:text-brand-400" /> Click-Through Rate
          </span>
          <div className="text-2xl font-bold text-slate-900 dark:text-white">
            {performance?.overall_ctr || 0.0}%
          </div>
          <span className="text-[11px] text-slate-500 dark:text-slate-400">Industry benchmark: 1.5%</span>
        </div>

        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-1 shadow-sm dark:shadow-none">
          <span className="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
            <Target className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400" /> Conversion Rate
          </span>
          <div className="text-2xl font-bold text-slate-900 dark:text-white">
            {performance?.overall_cvr || 0.0}%
          </div>
          <span className="text-[11px] text-slate-500 dark:text-slate-400">From click to lead/user</span>
        </div>

        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-1 shadow-sm dark:shadow-none">
          <span className="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
            <TrendingUp className="w-3.5 h-3.5 text-purple-500 dark:text-purple-400" /> Blended CAC
          </span>
          <div className="text-2xl font-bold text-slate-900 dark:text-white">
            {formatCurrency(performance?.overall_cac, currencyCode)}
          </div>
          <span className="text-[11px] text-slate-500 dark:text-slate-400">Per acquired conversion</span>
        </div>
      </div>
              ...
              {campaigns.length === 0 ? (
                ...
                campaigns.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="space-y-1">
                        <span className="text-slate-800 dark:text-slate-200 font-medium">
                          {formatCurrency(c.spend, currencyCode)} / {formatCurrency(c.budget, currencyCode)}
                        </span>
                    ...
                    <td className="py-3 px-4 text-slate-500 dark:text-slate-400">{c.impressions.toLocaleString()}</td>
                    <td className="py-3 px-4 text-slate-500 dark:text-slate-400">{c.clicks.toLocaleString()}</td>
                    <td className="py-3 px-4 font-bold text-emerald-600 dark:text-emerald-400">{c.conversions}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800 mb-4">
              <h3 className="text-base font-bold text-slate-900 dark:text-white">New Campaign Launch</h3>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <Input
                label="Campaign Name"
                placeholder="e.g. Q3 Growth Hacker Launch"
                value={name}
