...
export const AppLayout: React.FC = () => {
  ...
  if (isAuthLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#1e1e1e] text-slate-500 dark:text-[#9da7b3]">
        <div className="flex flex-col items-center gap-3">
          <svg className="animate-spin h-8 w-8 text-blue-600 dark:text-[#007acc]" fill="none" viewBox="0 0 24 24">
  ...
  return (
    <SidebarProvider>
      <div className="h-screen max-h-screen overflow-hidden flex flex-col bg-slate-50 dark:bg-[#1e1e1e] text-slate-900 dark:text-[#e6edf3] transition-colors duration-150">
        <Navbar />
        <div className="flex-1 flex overflow-hidden w-full h-[calc(100vh-4rem)]">
          <Sidebar />
          <main className="flex-1 overflow-y-auto overscroll-contain p-4 sm:p-6 bg-slate-100/60 dark:bg-[#1e1e1e]">
            <Outlet />
          </main>
        </div>
